const chawenje = {
  ...
  genres: [
    // pick two, first is primary, second is secondary
    (rockMetal = {
      subgenres: [
        "power metal",
        "metalcore",
  ...
  setOthers() {
    const { philosophies, modes, timeSigs, vibes, constraints, checkboxes } =
      this;
    ...
    if (checkboxes[5].checked) {
      ...
      this.params[this.params.length - 1] =
        "· constraints: " + usedConstraints.join("; ");
    }
  ...
  setParams() {
    // fills params array with one element of each, then strips down params
    this.genreOrStyle();
    this.setOthers();
  },
  ...
  generate() {
    // prints stripped down array with template message
  ...
  setSliders() {
    ...
    const constraintsNumSlider = document.querySelectorAll(
      'input[type="range"]'
    )[1]; // element to display current value of
